...
signUpForm.addEventListener('submit', function(event) {
    ...
    const url = 'https://cabby.github.io.com/api/signup';
    const options = {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
const returnLocationInput = document.querySelector('#return-location');
...
searchForm.addEventListener('submit', function(event) {
    ...
    // Make API request to search for cars
    const url = `https://cabby.github.io.com/api/cars?pickupLocation=${pickupLocation}&pickupDate=${pickupDate}&returnLocation=${returnLocation}&returnDate=${returnDate}`;
    fetch(url)
        .then(response => response.json())
        .then(data => {
          ...
          `;
                ...
                searchResults.innerHTML = '<p>No cars found. Please try a different search.</p>';
            }

            // Listen for rent button clicks
...
function rentCar(carId) {
    // Make API request to rent car
    const url = `https://cabby.github.io.com/api/cars/${carId}/rent`;
    const options = {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
    ...
    fetch(url, options)
        ...
        <p>You have rented a ${data.make} ${data.model} from ${data.pickupDate} to ${data.returnDate}.</p>
      `;
        })
        .catch(error => {
            console.error(error);
